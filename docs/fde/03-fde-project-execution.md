...
## 三、Git 的提交纪律：只提交阶段性成果

第三条纪律是关于 Git 的：**使用一定要谨慎，有了阶段性成果才提交。**

这听起来和主流的「频繁提交」相反，但 FDE 项目的逻辑不一样：**Git 历史后面是要用来回溯、用来改东西的。** 客户说「回到周三那版」、团队问「这个改动是什么时候引入的」，都依赖提交历史是干净的。

如果历史里塞满了试错——「试一下这个参数」「回滚刚才的改动」「再试一次」——那这条历史就废了：关键节点被噪音淹没，想找一个「能跑的版本」要在几十个提交里翻。

我们的原则是：

- **小的试错不提交**。调参、试样式、验证想法，这些动作留在本地，不进入历史
- **阶段性成果才提交**。一个模块跑通了、一个版本能演示了，这时候提交，提交信息写清楚这个版本「是什么、能干什么」
- **提交即里程碑**。每一个提交都应该是「如果明天项目出问题，我可以回到这里」的点

一句话：**Git 历史不是草稿纸，是档案。** 草稿可以乱，档案必须每一笔都有意义。
